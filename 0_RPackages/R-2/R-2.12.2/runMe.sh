...
printf '\n%s\n' 'Ram-cache and Swap Cleared'
echo 3 > /proc/sys/vm/drop_caches
retry cp configurationFile.R $pathSharedfoldDock/R-2.12.2_toBeInstalled/libraryInstall.R
sync
swapoff -a
swapon -a
printf '\n%s\n' 'Ram-cache and Swap Cleared'
...
if ! docker run  --platform linux/amd64 -tv $pathSharedfoldHost/R-2.12.2_toBeInstalled:/scratch $dockerTempName /scratch/1_libraryInstall.sh; then
    echo "Docker container failed!! check log"
    exit 1
fi
