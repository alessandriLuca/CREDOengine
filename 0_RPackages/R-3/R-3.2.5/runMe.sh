...
docker build . -t $1
cp -R ./R-3.2.5_toBeInstalled $pathSharedfoldDock
cp configurationFile.R $pathSharedfoldDock/R-3.2.5_toBeInstalled/libraryInstall.R
docker run -itv $pathSharedfoldHost/R-3.2.5_toBeInstalled:/scratch $1 /scratch/1_libraryInstall.sh # DEVE ESSERE IL PATH DI HOST, DEVE ESSERE LA SHARED FOLDER
